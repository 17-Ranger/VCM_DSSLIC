...
def pick_coco_exp(name, targetlist):
    ...
    anno_path = os.environ["DETECTRON2_ANNOTATIONS"]
...
def save_feature_map_MSFC(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    ...
    _save_feature_map_MSFC(filename, features_draw)

def _save_feature_map_MSFC(filename, features, debug=False):
    feat = [features["p0"].squeeze()]
    width_list = [4]
    height_list = [4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
...
def save_feature_map_P23_2ds(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    _save_feature_map_P23_2ds(filename, features_draw)

def _save_feature_map_P23_2ds(filename, features, debug=False):
    feat = [features["p2"].squeeze(), features["p3"].squeeze(), features["p4"].squeeze(), features["p5"].squeeze()]
    width_list = [32, 64, 64, 128]
    height_list = [8, 4, 4, 2]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
        big_blk = np.empty((0, blk.shape[2] * width))
...
def save_feature_map_2ds(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    _save_feature_map_2ds(filename, features_draw)

def _save_feature_map_2ds(filename, features, debug=False):
    feat = [features["p2"].squeeze(), features["p3"].squeeze(), features["p4"].squeeze(), features["p5"].squeeze()]
    width_list = [32, 32, 64, 128]
    height_list = [8, 8, 4, 2]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
        big_blk = np.empty((0, blk.shape[2] * width))
...
def save_feature_map_4ds(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    _save_feature_map_4ds(filename, features_draw)

def _save_feature_map_4ds(filename, features, debug=False):
    feat = [features["p2"].squeeze(), features["p3"].squeeze(), features["p4"].squeeze(), features["p5"].squeeze()]
    width_list = [64, 32, 64, 128]
    height_list = [4, 8, 4, 2]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
        big_blk = np.empty((0, blk.shape[2] * width))
...
def save_feature_map_onlyp2(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    del features_draw["p5"]
    del features_draw["p4"]
...
def _save_feature_map_onlyp2(filename, features, debug=False):
...
def save_feature_map_onlyp3(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    del features_draw["p5"]
    del features_draw["p4"]
    del features_draw["p2"]
    _save_feature_map_onlyp3(filename, features_draw)

def _save_feature_map_onlyp3(filename, features, debug=False):
    feat = [features["p3"].squeeze()]
    width_list = [16]
    height_list = [16]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
...
def save_feature_map_onlyp2_8channels(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    del features_draw["p5"]
    del features_draw["p4"]
    del features_draw["p3"]
    _save_feature_map_onlyp2_8channels(filename, features_draw)

def _save_feature_map_onlyp2_8channels(filename, features, debug=False):
    feat = [features["p2"].squeeze()]
    width_list = [2]
    height_list = [4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
...
def save_feature_map_onlyp3_8channels(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    del features_draw["p5"]
    del features_draw["p4"]
    del features_draw["p2"]
    _save_feature_map_onlyp3_8channels(filename, features_draw)

def _save_feature_map_onlyp3_8channels(filename, features, debug=False):
    feat = [features["p3"].squeeze()]
    width_list = [2]
    height_list = [4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
...
def save_feature_map_onlyp2_16channels(filename, features):
    features_draw = features.copy()
    del features_draw["p6"]
    del features_draw["p5"]
    del features_draw["p4"]
    del features_draw["p3"]
    _save_feature_map_onlyp2_16channels(filename, features_draw)

def _save_feature_map_onlyp2_16channels(filename, features, debug=False):
    feat = [features["p2"].squeeze()]
    width_list = [4]
    height_list = [4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
...
def save_feature_map_p345(filename, features):
    features_draw = features.copy()
    del features_draw["p2"]
...
def save_feature_map_p45(filename, features):
    features_draw = features.copy()
    del features_draw["p2"]
    del features_draw["p3"]
    _save_feature_map_p45(filename, features_draw)

def _save_feature_map_p45(filename, features, debug=False):
    feat = [features["p4"].squeeze(), features["p5"].squeeze()]
    width_list = [16, 32]
    height_list = [16, 8]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
...
def save_feature_map_p23_64c(filename, features):
    features_draw = features.copy()
    del features_draw["p4"]
    del features_draw["p5"]
    _save_feature_map_p23_64c(filename, features_draw)

def _save_feature_map_p23_64c(filename, features, debug=False):
    feat = [features["p2"].squeeze(), features["p3"].squeeze()]
    width_list = [8, 16]
    height_list = [8, 4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
    for blk, width, height in zip(feat, width_list, height_list):
...
def save_feature_map_p45_64c(filename, features):
    features_draw = features.copy()
    del features_draw["p2"]
    del features_draw["p3"]
    _save_feature_map_p45_64c(filename, features_draw)

def _save_feature_map_p45_64c(filename, features, debug=False):
    feat = [features["p4"].squeeze(), features["p5"].squeeze()]
    width_list = [8, 16]
    height_list = [8, 4]
    tile_big = np.empty((0, feat[0].shape[2] * width_list[0]))
